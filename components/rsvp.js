...
const RSVP = () => {
  ...
  return (
    <div>
      <h1 className={`${italianno.className} text-[3.5rem] flex justify-center my-7 ${isVisible1 ? "animate-slideInDown" : "opacity-0"}`} ref={ref1}>
        When and Where
      </h1>
      <p className={`flex justify-center ${isVisible1 ? "animate-spinnerGrow" : "opacity-0"}`}>
        Saturday, 17 July 2027, 11:00 AM - 03.00 PM <br /> Avalon Castle, Cockatoo VIC 3781, Australia
      </p>
      <div className={`flex flex-col mt-4 bg-white shadow-md shadow-black/25 w-max h-max mx-auto ${isVisible2 ? "animate-spinnerGrow" : "opacity-0"}`} ref={ref2}>
